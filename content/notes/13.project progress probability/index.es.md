...
Si los intentos son independientes con probabilidad $p$, entonces

$$
P(N \le n) = 1 - (1-p)^n,
$$

lo que crece monótonamente con $n$. Esto muestra que la probabilidad de concreción aumenta al invertir más intentos, y que la curva $P(N \le n)$ proporciona un modo de acotar la cantidad de recursos necesarios en promedio.

Cuando los intentos no son independientes y se definen probabilidades condicionales $p_k$,

$$
P(N \le n) = 1 - \prod_{k=1}^{n} (1-p_k),
$$

reflejando cómo la estructura del proyecto puede favorecer o dificultar la concreción. Si $p_k$ aumenta con $k$, la persistencia se vuelve más eficaz; si disminuye, el progreso es más incierto.
...
p \le p_n
$$
cada intento fallido ($X_k=0$) incrementa la probabilidad condicional del siguiente. *La persistencia incorpora aprendizaje*.

Cuando la sucesión $\{p_n\}_{n\in \mathbb{N}}$ es decreciente y además 
$$
p \ge p_n
$$
cada intento fallido reduce la probabilidad futura. *La persistencia acumula ignorancia*.
